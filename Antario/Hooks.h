...
private:
	...
	std::unique_ptr<HolaHook> pClientHook;
    std::unique_ptr<HolaHook> pClientModeHook;
    std::unique_ptr<HolaHook> pSurfaceHook;
	std::unique_ptr<HolaHook> pPanelHook;
	std::unique_ptr<HolaHook> pRenderViewHook;
	std::unique_ptr<ShadowVTManager> pPlayerHook[65];
	std::unique_ptr<HolaHook> pModelHook;
...
public:
	...
	HolaHook() : class_base(nullptr), method_count(0), shadow_vtable(nullptr), original_vtable(nullptr) {}
	//VTManager(void *base) : class_base(base), method_count(0), shadow_vtable(nullptr), original_vtable(nullptr) {}

	HolaHook(void* base) { Setup(base); }
