...
public:
	...
	void SetAngle2(Vector wantedang)
	{
		...
		static SetAngleFn SetAngle = (SetAngleFn)((DWORD)Utils::FindSignature("client_panorama.dll", "55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1"));
	...
	{
		return (float*)((uintptr_t)this + 0xA40C);
	}
	...
	float MaxDesyncDelta() {
		auto animstate = uintptr_t(this->AnimState());
		auto LocalPlayer = this;
		float speed = fmin(LocalPlayer->GetVelocity().Length(), 260.0f);
		float m_flSpeed = speed;
		float m_flRunningSpeed = m_flSpeed / (260 * 0.520f);
		float flRunningSpeed = max(0, min(1, *reinterpret_cast<float*> (animstate + 0xFC)));
		float m_flGroundFraction = *(float*)(LocalPlayer->AnimState() + 0x11C);
		float flYawModifier = (((m_flGroundFraction * -0.3f) - 0.2f) * flRunningSpeed) + 1.0f;
		float flMaxYawModifier = flYawModifier * 58;
	...
	Vector GetBonePosition2(int iBone);
};
class viewmodel_t;
