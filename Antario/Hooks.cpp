...
void Hooks::Init()
{
    ...
    // HolaHooks
	g_Hooks.pClientHook     = std::make_unique<HolaHook>(g_pClientDll);
    g_Hooks.pClientModeHook = std::make_unique<HolaHook>(g_pClientMode);
    g_Hooks.pSurfaceHook	= std::make_unique<HolaHook>(g_pSurface);
	g_Hooks.pPanelHook		= std::make_unique<HolaHook>(g_pPanel);
	g_Hooks.pRenderViewHook = std::make_unique<HolaHook>(g_pRenderView);
	g_Hooks.pModelHook = std::make_unique<HolaHook>(g_pModelRender);
	...
	Globals::KillCounterFont = g_pSurface->FontCreate();

	g_pSurface->SetFontGlyphSet(Globals::CourierNew, "Twiddlybitz", 13, 500, 0, 0, FONTFLAG_DROPSHADOW);
	g_pSurface->SetFontGlyphSet(Globals::SmallText, "Twiddlybitz", 9, 200, 0, 0, FONTFLAG_OUTLINE);
	g_pSurface->SetFontGlyphSet(Globals::WeaponESP, "Visitor TT2 BRK", 10, 300, 0, 0, FONTFLAG_NONE | FONTFLAG_OUTLINE);
	g_pSurface->SetFontGlyphSet(Globals::WeaponIcon, "undefeated", 13, 400, 0, 0, FONTFLAG_ANTIALIAS | FONTFLAG_OUTLINE);
	g_pSurface->SetFontGlyphSet(Globals::TabFont, "Twiddlybitz", 16, 200, 0, 0, FONTFLAG_DROPSHADOW);
	g_pSurface->SetFontGlyphSet(Globals::IndicatorFont, "Verdana", 15, 15, 0, 0, FONTFLAG_NONE);
	g_pSurface->SetFontGlyphSet(Globals::KillCounterFont, "Arial", 35, 700, 0, 0, 0);
...
bool __fastcall Hooks::CreateMove(IClientMode* thisptr, void* edx, float sample_frametime, CUserCmd* pCmd)
{
	...
	//g_resolver_11.OnCreateMove11();
	g_Resolver3.OnCreateMove3();
	g_Resolver11.OnCreateMove11();
	...
	if (c_config::get().misc_antiuntrusted)
	{
		g_Math.ClampAngles(Globals::pCmd->viewangles);
	}
...
void __stdcall Hooks::FrameStageNotify(ClientFrameStage_t curStage)
{
	...
	if (!g_pEngine->IsConnected() && !Globals::LocalPlayer && g_pEngine->IsInGame())
		return;
	g_Misc.Thirdperson_FSN(curStage);

	g_Resolver.FrameStage(curStage);
	g_Resolver3.FrameStage3(curStage);
	g_Resolver11.FrameStage11(curStage);
	//g_LagComp.FrameStage(curStage);
	...
	if (Globals::LocalPlayer) {
		if (Globals::LocalPlayer->IsAlive()) {
			...
			*/
			...
			int FAKE_G = Globals::LocalPlayer->DesyncValue() / -58.f * 255;
			...
			if (Show_Choke) {
				g_pSurface->DrawT(15 + (CHOKESIZE.right / 2), 560 - 14, Color(255, 255, 255, 255), Globals::CourierNew, true, "CHOKE");
				g_pSurface->FilledRect(15, 560, CHOKESIZE.right + 20, 15, Color(0, 0, 0, 185));
				g_pSurface->OutlinedRect(15, 560, CHOKESIZE.right + 20, 15, Color(0, 0, 0, 255));//
				g_pSurface->FilledRect(15, 560, ChokedWidth, 15, Color(CHOKE_R, CHOKE_G, CHOKE_B, 255));//
				g_pSurface->OutlinedRect(15, 560, ChokedWidth, 15, Color(0, 0, 0, 255));//

				IndicatorPos += CHOKESIZE.right;
			}

			if (Show_Fake) {
				g_pSurface->DrawT(15 + (CHOKESIZE.right / 2), 590 - 14, Color(255, 255, 255, 255), Globals::CourierNew, true, "FAKE");
				g_pSurface->FilledRect(15, 590, CHOKESIZE.right + 20, 15, Color(0, 0, 0, 185));
				g_pSurface->OutlinedRect(15, 590, CHOKESIZE.right + 20, 15, Color(0, 0, 0, 255));
				g_pSurface->FilledRect(15, 590, DesyncWidth, 15, Color(FAKE_R, FAKE_G, FAKE_B, 255));
				g_pSurface->OutlinedRect(15, 560, DesyncWidth, 15, Color(0, 0, 0, 255));//
...
void watermark() // who needs gettextsize LOL
{
	int width, height;
	...
	ss << "holahooks [BETA]" << " | " << name << " | fps: " << fps << " | TIME: " << __TIME__ << " | DATE: " <<__DATE__;
	ss69 << "holahooks [BETA]" << " | " <<__TIME__ << " | " << __DATE__;

	RECT TextSize = g_pSurface->GetTextSizeRect(Globals::CourierNew, ss.str().c_str());
	RECT TextSize69 = g_pSurface->GetTextSizeRect(Globals::CourierNew, ss69.str().c_str());


	if (!Globals::LocalPlayer || !g_pEngine->IsInGame() || !g_pEngine->IsConnected())
	{

		/*---------------------------------------------------------------------------------------------------------*/
		g_pSurface->FilledRect(width - (TextSize69.right + 20), 25, TextSize69.right + 10, 2, Color(101, 101, 101, 255));
		/*--------------------------------------------------------------------------------------------------------*/

		/*---------------------------------------------------------------------------------------------------------*/
		g_pSurface->FilledRect(width - (TextSize69.right + 20), 3, TextSize69.right + 10, 2, Color(101, 101, 101, 255));
		/*--------------------------------------------------------------------------------------------------------*/

		g_pSurface->AutoGradientRect(width - (TextSize69.right + 20), 5, TextSize69.right + 10, 20, Color(35, 35, 35, 200));
		g_pSurface->OutlinedRect(width - (TextSize69.right + 20), 5, TextSize69.right + 10, 20, Color(101, 101, 101, 255));
		g_pSurface->DrawT(width - (TextSize69.right + 15), 8, Color(255, 255, 255), Globals::CourierNew, false, ss69.str().c_str());
	}
	else
	{
		/*---------------------------------------------------------------------------------------------------------*/
		g_pSurface->FilledRect(width - (TextSize.right + 20), 25, TextSize.right + 10, 2, Color(101, 101, 101, 255));
		/*--------------------------------------------------------------------------------------------------------*/

		/*---------------------------------------------------------------------------------------------------------*/
		g_pSurface->FilledRect(width - (TextSize.right + 20), 3, TextSize.right + 10, 2, Color(101, 101, 101, 255));
		/*--------------------------------------------------------------------------------------------------------*/

		g_pSurface->AutoGradientRect(width - (TextSize.right + 20), 5, TextSize.right + 10, 20, Color(35, 35, 35, 200));
		g_pSurface->OutlinedRect(width - (TextSize.right + 20), 5, TextSize.right + 10, 20, Color(101, 101, 101, 255));
		g_pSurface->DrawT(width - (TextSize.right + 15), 8, Color(255, 255, 255), Globals::CourierNew, false, ss.str().c_str());
	}
}
...
void DrawAAIndicators()
{

	if (!g_pEngine->IsConnected() || !g_pEngine->IsInGame() || !Globals::LocalPlayer)
		return;

	static const auto real_color = Color(0, 255, 0, 200);
	static const auto fake_color = Color(50, 50, 255, 200);
	static const auto lby_color = Color(255, 0, 0, 200);
	...
	if (c_config::get().antiaim_indicator)
	{
		...
		Vector local_origin = Globals::LocalPlayer->GetAbsOrigin();

		Vector2D screen1, screen2;

		Utils::WorldToScreen(local_origin, screen1);

		Utils::WorldToScreen(g_Math.get_rotated_position(local_origin, Globals::FakeAngle.y, distance), screen2);
		g_pSurface->Line(screen1.x, screen1.y, screen2.x, screen2.y, fake_color);
		g_pSurface->DrawT(screen2.x, screen2.y, fake_color, Globals::WeaponESP, true, "FAKE");

		Utils::WorldToScreen(g_Math.get_rotated_position(local_origin, Globals::RealAngle.y, distance), screen2);
		g_pSurface->Line(screen1.x, screen1.y, screen2.x, screen2.y, real_color);
		g_pSurface->DrawT(screen2.x, screen2.y, real_color, Globals::WeaponESP, true, "REAL");

		Utils::WorldToScreen(g_Math.get_rotated_position(local_origin, Globals::LocalPlayer->GetLowerBodyYaw(), distance), screen2);
		g_pSurface->Line(screen1.x, screen1.y, screen2.x, screen2.y, lby_color);
		g_pSurface->DrawT(screen2.x, screen2.y, lby_color, Globals::WeaponESP, true, "LBY");
	}
}
	...
	if (panelID == vguiPanel)
	{
		...
		if (c_config::get().visuals_enabled) {
			if (g_pEngine->IsConnected())
			{
				...
				//c_visuals::get().QuakeKillCounter();
				//LogEvents::Draw();
			}
		}
		...
		for (int i = 0; i <= 2048; i++) {
			...
			if (e->GetClientClass()->ClassID == (int)EClassIds::CPlantedC4) {
				ccsbomb* bomb = (ccsbomb*)e;
				if (bomb) {

					if (!c_config::get().bom_esp_timer || bomb->bomb_defused()) continue;

					float c4_clock = bomb->c4_blow_time() - g_pGlobalVars->curtime;
					char buf[24];
					sprintf_s(buf, sizeof(buf) - 1, "Bomb Time: %.1f", c4_clock);
					g_pSurface->DrawT(890 + 20, 90, Color(220, 220, 220), Globals::CourierNew, false, c4_clock >= 0 ? buf : "Bomb Time: 0.0");
					g_pSurface->FilledRect(619 + 20, 109, 600, 12, Color(0, 0, 0, 180));
					g_pSurface->FilledRect(620 + 20, 110, c4_clock * 15, 10, Color(0, 255, 0, 200));


					float c4_defuse = bomb->c4_defuse_count_down() - g_pGlobalVars->curtime;
					char buff[24];
					sprintf_s(buff, sizeof(buff) - 1, "Defuse Time: %.1f", c4_defuse);
					if (Globals::LocalPlayer->defusing() || defuse) {
						auto multi = 0;
						if (Globals::LocalPlayer->has_defuser() || ent_has_defuser)
							multi = 100;
						else
							multi = 60;

						g_pSurface->DrawT(890 + 20, 135, Color(220, 220, 220), Globals::CourierNew, false, c4_defuse >= 0 ? buff : "Defuse Time: 0.0");
						g_pSurface->FilledRect(619 + 20, 154, 600, 12, Color(0, 0, 0, 180));
						g_pSurface->FilledRect(620 + 20, 155, c4_defuse * multi, 10, Color(0, 255, 0, 200));
					}
				}
			}
		}
...
void Event::FireGameEvent(IGameEvent* event)
{
	...
	if (!strcmp(event->GetName(), "round_start") || !Globals::LocalPlayer->IsAlive())
	{
	...
	if (!strcmp(name, "player_hurt")) {
		...
		if (c_config::get().aimbot_enabled) {
			...
			if (hittedplayer && hittedplayer->EntIndex() > 0 && hittedplayer->EntIndex() < 64)
			{
				if (Globals::LocalPlayer && hittedplayer != Globals::LocalPlayer)
					CapsuleOverlay(hittedplayer, Color(155, 10, 10, 255), 2);
			}
		}

		if (c_config::get().KillCounter)
		{
			Globals::killtime = g_pGlobalVars->realtime;
			Globals::kills++;
		}

		Globals::globalAlpha = g_pGlobalVars->curtime;
		...
		if (c_config::get().hitmarker) {
			...
			case 2: PlaySoundA(bubble, nullptr, SND_MEMORY | SND_ASYNC); break;
			}
...
void __fastcall Hooks::SceneEnd(void *ecx, void *edx)
{
	...
	for (int i = 1; i < g_pEngine->GetMaxClients(); ++i)
	{
		...
		*/
		...
		float desync_color[3] = { 150 / 255.f, 0 / 255.f, 0 / 255.f };
		if (IsEnemy) {
		...
		else if (IsLocal && c_config::get().local_chams) {
	    ...
	    if (IsLocal && c_config::get().desync_chams && c_config::get().antiaim_enabled && c_config::get().fakelag)
		{
			Vector OrigAng;
			IMaterial* Dagtag = g_pMaterialSys->FindMaterial("dev/glow_armsrace", "Other textures");
			OrigAng = Globals::LocalPlayer->GetAbsAngles();
			Globals::LocalPlayer->SetAngle2(Vector(0, Globals::pCmd->viewangles.y - Globals::LocalPlayer->DesyncValue(), 0));
			g_pRenderView->SetBlend(50 / 100.f);
			g_pModelRender->ForcedMaterialOverride(Dagtag);
			g_pRenderView->SetColorModulation(desync_color);
			pPlayerEntity->DrawModel(0x00000001, 255);
			Globals::LocalPlayer->SetAngle2(OrigAng);
		}
		g_pModelRender->ForcedMaterialOverride(nullptr);
	}
