...
void Resolver3::Resolve(C_BaseEntity* pEnt)
{
	...
	if (pEnt != Globals::LocalPlayer && (pEnt->GetTeam() != Globals::LocalPlayer->GetTeam()) && c_config::get().aimbot_resolver == 3 /*&& !( GetAsyncKeyState( g_Menu.Config.override_resolver_key ) && g_Menu.Config.ResolverOverride && Globals::LocalPlayer && Globals::LocalPlayer->IsAlive())*/ || pEnt != Globals::LocalPlayer && c_config::get().aimbot_resolver == 3 /*&& !( GetAsyncKeyState( g_Menu.Config.override_resolver_key ) && g_Menu.Config.ResolverOverride && Globals::LocalPlayer && Globals::LocalPlayer->IsAlive())*/) {
		...
		else if (Globals::MissedShots[pEnt->EntIndex()] < 2) {
		...
		Globals::MissedShots[pEnt->EntIndex()] = Globals::MissedShots[pEnt->EntIndex()] % 6; // so it just doesn't brute all the fucking time, that's dumb.
	}
}

void Resolver3::OnCreateMove3() // cancer v2
{
	if (!c_config::get().aimbot_resolver == 3)
		return;
	...
	for (int i = 1; i < g_pEngine->GetMaxClients(); ++i)
	{
		...
		{
			UseFreestandAngle3[i] = false;
			continue;
		}

		if (abs(pPlayerEntity->GetVelocity().Length2D()) > 29.f)
			UseFreestandAngle3[pPlayerEntity->EntIndex()] = false;

		if (abs(pPlayerEntity->GetVelocity().Length2D()) <= 29.f && !UseFreestandAngle3[pPlayerEntity->EntIndex()])
		{
			...
			for (int side = 0; side < 2; side++)
			{
				...
				if (g_Autowall.CanHitFloatingPoint(OriginAutowall, ViewPoint))
				{
					...
					{
						HitSide1 = true;
						FreestandAngle3[pPlayerEntity->EntIndex()] = 90;
					}
					else if (side == 1)
					{
						HitSide2 = true;
			...
			if (Autowalled)
			{
				if (HitSide1 && HitSide2)
					...
					UseFreestandAngle3[pPlayerEntity->EntIndex()] = true;
			}
		}

		// bad idea but ill try this why not
		if (UseFreestandAngle3[pPlayerEntity->EntIndex()] && Globals::MissedShots[pPlayerEntity->EntIndex()] < 4) {
			pPlayerEntity->SetEyeAngles(Vector(pPlayerEntity->GetEyeAngles().x, pPlayerEntity->GetEyeAngles().y + FreestandAngle3[pPlayerEntity->EntIndex()], pPlayerEntity->GetEyeAngles().z));
		}
	}
}

void HandleHits3(C_BaseEntity* pEnt)
{
	...
	if (Globals::Shot[pEnt->EntIndex()])
	{
		...
		if (g_pGlobalVars->curtime > predTime[pEnt->EntIndex()] && !Globals::Hit[pEnt->EntIndex()])
		{
			...
			Globals::Shot[pEnt->EntIndex()] = false; // missed shot due to spread (?) - no
		}
	}
	else
...
{
	if (!c_config::get().aimbot_resolver == 3 || !c_config::get().ResolverOverride)
		return;

	if (!GetAsyncKeyState(c_config::get().override_resolver_key))
		return;
	...
	auto at_target_yaw = g_Math.CalcAngle(pEnt->GetOrigin(),
		Globals::LocalPlayer->GetOrigin()).y;
...
void Resolver3::FrameStage3(ClientFrameStage_t stage)
{
