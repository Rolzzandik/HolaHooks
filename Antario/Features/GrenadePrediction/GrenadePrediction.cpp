		...
		if ((type) && path.size() > 1)
		{
			...
			for (auto it = path.begin(), end = path.end(); it != end; ++it)
			{
				if (ExplectedWorldToScrean(prev, nadeStart) && ExplectedWorldToScrean(*it, nadeEnd))
					DrawLine((int)nadeStart.x, (int)nadeStart.y, (int)nadeEnd.x, (int)nadeEnd.y, rainbow);
				prev = *it;
			}
			for (auto it = others.begin(), end = others.end(); it != end; ++it) {
				...
				if (ExplectedWorldToScrean(prevoth, nadeStart2) && ExplectedWorldToScrean(it->first, nadeEnd2)) {
					g_pSurface->FilledRect((int)nadeStart2.x - 2, (int)nadeStart2.y - 2, 4, 4, rainbow);
				}
			...
			//RENDER::DrawFilled3DBox(others.rbegin()->first, 4, 4, Color(255, 0, 0, 200), Color(0, 135, 255, 200));
		}
	}
}
