...
{
	m_fbestfov = c_config::get().legit_aimbot_fov;
...
/*float LegitAimbot::MaxRotation(C_AnimState* AnimState)
{
	auto animstate = uintptr_t(AnimState);
	float duckammount = *(float*)(animstate + 0xA4);
	float speedfraction = std::max(0.fm std::min(*reinterpret_cast<float*>(animstate + 0xF8), 1.f));
